...
$sql = 'SELECT s.id, s.purchase_date, s.sim_no, s.sim_type, s.sim_validity_id, s.notes, v.months AS sim_validity_months, s.status, sa.allocation_date, sa.owner_type, sa.owner_id, sa.payment_status, sa.software, CASE WHEN sa.owner_type = "dealer" THEN dl.dealer_name WHEN sa.owner_type = "technician" THEN t.technician_name END AS owner_name FROM sims s LEFT JOIN sim_validities v ON v.id = s.sim_validity_id LEFT JOIN stock_allocations sa ON sa.sim_id = s.id AND sa.id = (SELECT MAX(id) FROM stock_allocations WHERE sim_id = s.id) LEFT JOIN dealers dl ON dl.id = sa.owner_id AND sa.owner_type = "dealer" LEFT JOIN technicians t ON t.id = sa.owner_id AND sa.owner_type = "technician"';

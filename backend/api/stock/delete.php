...
try {
    $stmt = $conn->prepare('SELECT * FROM stock_allocations WHERE owner_type = ? AND owner_id = ? FOR UPDATE'); $stmt->bind_param('si', $ownerType, $ownerId); $stmt->execute(); $result = $stmt->get_result(); $deviceIds = []; $simIds = []; $allocationRows = [];
    while ($row = $result->fetch_assoc()) { $allocationRows[] = $row; if ($row['device_id']) $deviceIds[] = (int)$row['device_id']; if ($row['sim_id']) $simIds[] = (int)$row['sim_id']; } $stmt->close();
    foreach ($allocationRows as $allocation) writeDeleteSnapshot($conn, (int) $allocation['id'], 'Stock Allocation', $allocation, $currentUser);

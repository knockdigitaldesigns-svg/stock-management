...
const StockAllocationEditModal = ({ allocationId, onClose, onSuccess }) => {
    ...
    const [form, setForm] = useState({ owner_type: '', owner_id: '', allocation_type: '', allocation_date: '', activation_date: '', sim_validity_id: '', deactivation_date: '', software: '', total_amount: '', amount_paid: '', payment_mode: '', transaction_id: '', notes: '' });
    ...
    useEffect(() => {
        Promise.all([
            ...
            const validityResponse = await api.get('/sim_validities/list.php');
            setValidities(validityResponse.data.data?.validities || []);
    ...
    const isDealerSim = isSim && form.owner_type === 'dealer';
    const validityMonths = Number(validities.find((validity) => Number(validity.id) === Number(form.sim_validity_id))?.months || allocation?.sim_validity_months || 0);
    const expiryDate = isSim && form.activation_date && validityMonths
        ? (() => { const date = new Date(`${form.activation_date}T00:00:00`); date.setMonth(date.getMonth() + validityMonths); return date.toISOString().slice(0, 10); })()
        : '';
    ...
    const save = async () => {
        ...
        try {
            ...
            const response = await api.post('/stock/allocation_update.php', { allocation_id: allocationId, owner_type: form.owner_type || ownerType, owner_id: Number(ownerId), allocation_type: form.allocation_type, allocation_date: form.allocation_date, activation_date: isDealerSim ? form.activation_date : undefined, sim_validity_id: isDealerSim ? Number(form.sim_validity_id) : undefined, deactivation_date: isDealerSim ? form.deactivation_date : undefined, software: form.software, total_amount: Number(form.total_amount || 0), amount_paid: Number(form.amount_paid || 0), payment_mode: form.payment_mode, transaction_id: transactionRequired ? form.transaction_id.trim() : null, notes: form.notes });
    ...
    return <Modal isOpen title="Edit Stock Allocation" onClose={onClose} maxWidth="760px" footer={<><button type="button" className="btn btn-outline" onClick={onClose} disabled={saving}>Cancel</button><button type="button" className="btn btn-primary" onClick={save} disabled={loading || saving || !allocation}>{saving ? 'Saving...' : 'Save Changes'}</button></>}>
        ...
        {allocation && <div style={{ display: 'grid', gap: '1rem' }}>
            ...
            {isDealerSim && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '0.75rem' }}>
                <div className="form-group"><label className="form-label">Activation Date</label><DateInput className="form-control" value={form.activation_date} onChange={(value) => update('activation_date', value)} /></div>
                <div className="form-group"><label className="form-label">SIM Validity</label><select className="form-control" value={form.sim_validity_id} onChange={(event) => update('sim_validity_id', event.target.value)}><option value="">Select validity</option>{validities.filter((validity) => !validity.status || String(validity.status).toLowerCase() === 'active').map((validity) => <option key={validity.id} value={validity.id}>{validity.months} Months</option>)}</select></div>
                <div className="form-group"><label className="form-label">Expiry Date</label><input className="form-control" value={expiryDate ? formatDate(expiryDate) : '-'} readOnly /></div>
                <div className="form-group"><label className="form-label">Deactivation Date</label><DateInput className="form-control" value={form.deactivation_date} onChange={(value) => update('deactivation_date', value)} /></div>
                <div className="form-group"><label className="form-label">SIM Status</label><input className="form-control" value={form.deactivation_date ? 'Deactive' : form.activation_date ? 'Active' : 'Available'} readOnly /></div>
            </div>}
